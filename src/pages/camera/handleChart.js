...
export default function chart($) {
  ...
  const ws = new WebSocket('ws://192.168.50.216:8802');
  ...
  $('.online-user-list').addEventListener('click', event => {
    if (event.target.classList.contains('call-btn')) {
      ...
      const userId = event.target.parentNode.parentNode.dataset.userId;
      const chat = new Chat(ws, userId, $);
      currentChat = chat;
      chat.ring();
    }
  });
  ...
  $('.answer-call').addEventListener('click', event => {
    currentChat && currentChat.closeConnection();
    const chat = new Chat(ws, remoteUserId, $);
    chat.initConnect();
    currentChat = chat;
    ws.sendData({
      type: 'answerCall',
